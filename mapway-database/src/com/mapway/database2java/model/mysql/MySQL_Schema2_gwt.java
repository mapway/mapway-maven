...
public class MySQL_Schema2_gwt extends SchemaBase {
  ...
  public boolean fetchSchema() {
    ...
    // options.setSchemaInclusionRule(new RegularExpressionInclusionRule(this.getConfigure()
    // .getSchema()));
    //
    //
    ...
    // System.out.println(schema.getFullName() + "===>" + getConfigure().getSchema());
  ...
  public String exportTable(ITable table, Configure conf) {
    ...
    writeToFile(cf.getFilePath() + "/server/database/dao/", "I" + table.getName() + "DAO.java",
        sb.toString());
    ...
    writeToFile(cf.getFilePath() + "/server/database/dao/impl/", table.getName() + "DAOImpl.java",
        sb.toString());
    ...
    for (int i = 0; i < table.getColumns().getCount(); i++) {
      ...
      if (col.getJavaType().equals("String")) {
        ...
        out(sb, "\t\tsb.append(JSONHelper.escapeForXML(this.get" + col.getName()
            + "()==null?\"\":this.get" + col.getName() + "().toString()));");
  ...
  private void writeJavaAccess(ITable table, StringBuilder sb, Configure conf) {
    ...
    out(sb, " */");
    out(sb, "public class " + table.getName() + "DAOImpl" + " extends AccessBase implements I"
        + table.getName() + "DAO{\r\n");
    ...
    for (int i = 0; i < table.getColumns().getCount(); i++) {
      ...
      out(sb, "\t*/");
      out(sb,
          "public " + table.getJavaName() + " findBy" + col.getName() + "  (" + col.getJavaType()
              + " " + col.getName().toLowerCase() + ") throws SQLException{");
      ...
      for (int i = 0; i < table.getColumns().getCount(); i++) {
        ...
        if (col.isAuto()) {
          out(sb, "\t\t\tif(" + table.getJavaName().toLowerCase() + ".getChanged( " + i
              ...
              + "(rs.get" + col.getJavaType4() + "(index1++));}");
   ...
   * 
   * @see
   * com.mapway.database2java.model.base.SchemaBase#exportProcedures(com.mapway.database2java.model
   * .base.Configure)
   */
  @Override
  public void exportProcedures(Configure confProcedure) {
    ...
    for (int i = 0; i < this.getPackages().getCount(); i++) {
      ...
      for (int j = 0; j < procs.getCount(); j++) {
        ...
        imports.append("\r\nimport " + confProcedure.getPackage() + ".shared.modle.Arg_"
            + proc.getName() + ";");
    ...
    out(sb1, t2.toString());
    out(sb1, "}");
    writeToFile(confProcedure.getFilePath() + "/server/database/dao/" + "/impl", "ProceduresImpl"
        + ".java", sb.toString());
    writeToFile(confProcedure.getFilePath() + "/server/database/dao", "IProcedures" + ".java",
        sb1.toString());
  }
  ...
  private void exportProcedureArgument(Procedure func, Configure conf) {
    ...
    out(sb, "}");
    writeToFile(conf.getFilePath() + "/shared/modle", "Arg_" + func.getName() + ".java",
        sb.toString());
  }
    ...
    for (int i = 0; i < func.getArguments().getCount(); i++) {
      ...
      if (arg.getProperty().equals("IN/OUT")) {
        ...
        if (arg.getType().equals("REF CURSOR")) {
          ...
          out(sb, "\t\t\targ.set" + arg.getName() + "(cstm.get" + arg.getJavaType4() + "("
              + (i + 1) + "));");
        }
      }
    }
   ...
   * 
   * @see
   * com.mapway.database2java.model.base.SchemaBase#exportSpringConfigure(com.mapway.database2java
   * .model.base.Configure)
   */
  @Override
  public void exportSpringConfigure(Configure conf) {
    ...
    for (int i = 0; i < ts.getCount(); i++) {
      ITable t = ts.getAt(i);
      sb.append("\t<bean id=\"" + t.getName() + "DAO\" scope=\"prototype\"" + "\tclass=\""
          + conf.getPackage() + ".server.database.dao.impl." + t.getJavaAccessName() + "\">\r\n"
   ...
   * 
   * @see
   * com.mapway.database2java.model.base.SchemaBase#exportDwrConfigure(com.mapway.database2java.
   * model.base.Configure)
   */
  @Override
  public void exportDwrConfigure(Configure conf) {
    ...
    for (int i = 0; i < ts.getCount(); i++) {
      ITable t = ts.getAt(i);
      sb.append("\t\t<convert converter=\"bean\" " + "match=\"" + conf.getPackage()
          + ".shared.modle." + t.getJavaName() + "\"/>\r\n");
    }
