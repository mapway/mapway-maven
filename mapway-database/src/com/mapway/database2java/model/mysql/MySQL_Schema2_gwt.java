...
public class MySQL_Schema2_gwt extends SchemaBase {
	...
	public String exportTable(ITable table, Configure conf) {
		...
		writeToFile(cf.getFilePath() + "/shared/modle", table.getName()
				+ "Obj.java", sb.toString());

		sb = new StringBuilder();
		out(sb, getCopyright());
		writeJavaObjs(table, sb, cf);
		writeToFile(conf.getFilePath() + "/shared/modle", table.getName()
		...
		writeToFile(cf.getFilePath() + "/server/database/dao/",
				"I" + table.getName() + "DAO.java", sb.toString());
		...
		writeToFile(cf.getFilePath() + "/server/database/dao/impl/",
				table.getName() + "DAOImpl.java", sb.toString());
	...
	private void writeJavaInterface(ITable table, StringBuilder sb,
		...
		out(sb, "import " + conf.getPackage()
				+ ".server.database.base.ExecuteResult;");
		out(sb,
				"import " + conf.getPackage() + ".shared.modle."
						+ table.getJavaName() + ";");
		out(sb,
				"import " + conf.getPackage() + ".shared.modle."
						+ table.getJavaNames() + ";");
		...
		for (int i = 0; i < table.getColumns().getCount(); i++) {
			Column col = table.getColumns().getAt(i);
			out(sb, "\t/**");
			out(sb, "\t* 设置字段 " + col.getComment() + " [" + col.getName()
					+ "] 是否查询");
		...
		out(sb, "\t/**");
		out(sb, "\t* 根据主键获取" + table.getComment() + "[" + table.getName()
				+ "]中的单条记录");
		...
		if (table.hasPK() == true) {
			...
			out(sb, "\t*/");
			out(sb,
					"public " + table.getJavaName() + " selectByPK  ("
							+ param.toString() + ") throws SQLException;");

			out(sb, "\t/**");
			out(sb,
					"\t* 根据ORACLE 主键删除" + table.getComment() + "["
							+ table.getName() + "]中的单条记录");
	...
	private void writeJavaBean(ITable table, StringBuilder sb, Configure conf) {
		...
		for (int i = 0; i < table.getColumns().getCount(); i++) {
			...
			out(sb, "\t*/");
			out(sb, "\t public final static int INDEX_"
					+ c.getName().toUpperCase() + "=" + i + ";\r\n");
		}
		...
		for (int i = 0; i < table.getColumns().getCount(); i++) {
			...
			if (col.getJavaType().equals("String")) {
				...
				out(sb,
						"\t\tsb.append(JSONHelper.escapeForXML(this.get"
								+ col.getName() + "()==null?\"\":this.get"
				...
				out(sb,
						"\t\tsb.append(JSONHelper.escapeForXMLthis.get"
								+ col.getName() + "()==null?\"\":this.get"
								+ col.getName() + "().toString()));");
		...
		for (int i = 0; i < table.getColumns().getCount(); i++) {
			...
			if (col.getJavaType().equals("String")) {
				out(sb, "\t\tsb.append(\"\\\"\");");
				out(sb,
						"\t\tsb.append(JSONHelper.escape(this.get"
								+ col.getName() + "()==null?\"\":this.get"
								+ col.getName() + "().toString()));");
				out(sb, "\t\tsb.append(\"\\\"\");");
			} else if (col.getJavaType().equals("java.math.BigDecimal")) {
	...
	private void writeJavaObjs(ITable table, StringBuilder sb, Configure conf) {
		...
		out(sb, "		if(tag==null || tag.equals(\"\"))                 ");
		out(sb, "		{                                               ");
		out(sb, "			sb.append(\"<" + table.getName()
				+ "S \");                           ");
		out(sb, "		}                                               ");
		out(sb, "		else                                            ");
		out(sb, "		{                                               ");
		...
		out(sb, "		sb.append(\"\\\" >\\r\\n\");                          ");
		out(sb, "		                                                ");
		out(sb, "		Iterator<" + table.getJavaName()
				+ "> it=this.iterator();          ");
		out(sb, "		while(it.hasNext())                             ");
		out(sb, "		{                                               ");
		out(sb, "			" + table.getJavaName()
				+ " obj=it.next();                       ");
		out(sb, "			sb.append(\"\\t\");                              ");
	...
	private void writeJavaAccess(ITable table, StringBuilder sb, Configure conf) {
		...
		out(sb, "import " + conf.getPackage()
				+ ".server.database.base.AccessBase;");
		out(sb, "import " + conf.getPackage()
				+ ".server.database.base.ExecuteResult;");
		out(sb, "import " + conf.getPackage()
				+ ".server.database.base.IConnectionPool;\r\n");
		out(sb, "import java.util.ArrayList;");
		out(sb, "import " + conf.getPackage() + ".server.database.dao.I"
				+ table.getName() + "DAO;\r\n");
		...
		for (int i = 0; i < table.getColumns().getCount(); i++) {
			...
			if (col.getJavaType().equals("byte[]")) {
				...
				out(sb,
						"\t\tif(true ==m_searched[" + i + "])"
								+ "{ \r\n\t\t\trs.get" + col.getJavaType4()
								+ "(i);"

								+ "o.set" + col.getName()
								+ "(true==rs.wasNull()?null:rs.get"
								+ col.getJavaType4() + "(i));i++;\r\n\t\t}");
			} else {
				out(sb,
						"\t\tif(true ==m_searched[" + i + "]) o.set"
								+ col.getName() + "(rs.get"
		...
		if (table.hasPK() == true) {
			...
			out(sb,
					"\tboolean b=deleteByWhereTemplate(sql,"
							+ param3.toString() + " );");
			out(sb, "\treturn b;");
			out(sb, "\t}\r\n");
		}
		...
		for (int i = 0; i < table.getColumns().getCount(); i++) {
			...
			if (col.getJavaType().equals("byte[]")) {
				...
				out(sb,
						"\t\t\tblob" + String.valueOf(cur) + ".setBytes(1, "
								+ table.getJavaName().toLowerCase() + ".get"
								+ col.getName() + "());");
				out(sb,
						"\t\t\tif(" + table.getJavaName().toLowerCase()
								+ ".getChanged( " + i
		...
		for (int i = 0; i < table.getColumns().getCount(); i++) {
			...
			if (col.isPK())
				out(sb,
						"\t\t\tstatement.set" + col.getJavaType2() + "(index, "
								+ table.getJavaName().toLowerCase() + ".get"
								+ col.getName() + "());index++;");
		}
		out(sb, "\t\t\tb = statement.execute();");
	...
	public void exportProcedures(Configure confProcedure) {
		...
		for (int i = 0; i < this.getPackages().getCount(); i++) {
			...
			for (int j = 0; j < procs.getCount(); j++) {
				...
				imports.append("\r\nimport " + confProcedure.getPackage()
						+ ".shared.modle.Arg_" + proc.getName() + ";");
		...
		out(sb1, "package " + confProcedure.getPackage()
				+ ".server.database.dao;\r\n");
		out(sb1, imports.toString());
		out(sb1, "public interface  IProcedures{\r\n");
		out(sb1, t2.toString());
		out(sb1, "}");
		writeToFile(confProcedure.getFilePath() + "/server/database/dao/"
				+ "/impl", "ProceduresImpl" + ".java", sb.toString());
		writeToFile(confProcedure.getFilePath() + "/server/database/dao",
				"IProcedures" + ".java", sb1.toString());
	}
	...
	private void exportProcedureArgument(Procedure func, Configure conf) {
		...
		out(sb, "}");
		writeToFile(conf.getFilePath() + "/shared/modle",
				"Arg_" + func.getName() + ".java", sb.toString());
	}
	...
	public void exportSpringConfigure(Configure conf) {
		...
		String header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\r\n"
				...
				+ "	<bean id=\"MyPool\" class=\"" + conf.getGwtbase()
				+ ".server.database.base.MyPool\"\r\n"
	...
	public void exportDwrConfigure(Configure conf) {
		...
		for (int i = 0; i < ts.getCount(); i++) {
			ITable t = ts.getAt(i);
			sb.append("\t\t<convert converter=\"bean\" " + "match=\""
					+ conf.getPackage() + ".shared.modle." + t.getJavaName()
					+ "\"/>\r\n");
		}
